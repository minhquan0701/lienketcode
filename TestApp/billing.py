...
class BillingApp:
    def __init__(self, parent=None, readonly=False):
        # Nếu parent là None (chạy độc lập), tạo Tk() chính, ngược lại tạo Toplevel
        if parent is None:
            self.window = tk.Tk()
            self.is_root = True
        else:
        ...
        # (dùng file chung 'billing.db' để report_notify.py có thể đọc)
        self.conn = sqlite3.connect('billing.db')
        self.cursor = self.conn.cursor()
        self._setup_db()

        # Danh sách phòng (bạn có thể điều chỉnh nếu muốn)
        ...
        # Nếu chạy độc lập (parent is None) thì mainloop
        if self.is_root:
    ...
    def _save_room(self, room):
        ...
        self.cursor.execute("UPDATE billing SET total_amount=?, total_paid=?, payment_status=?, items_json=?, last_updated=? WHERE room=?",
                            (data['total_amount'], data['total_paid'], data['payment_status'], items_json, datetime.now().isoformat(), room))
        if self.cursor.rowcount == 0:
            self.cursor.execute("INSERT INTO billing (room, total_amount, total_paid, payment_status, items_json, last_updated) VALUES (?,?,?,?,?,?)",
                                (room, data['total_amount'], data['total_paid'], data['payment_status'], items_json, datetime.now().isoformat()))
        self.conn.commit()
    ...
    def _load_room_from_db(self, room):
        ...
        if row:
            ...
            self.cursor.execute(
                "INSERT INTO billing (room, total_amount, total_paid, payment_status, items_json, last_updated) VALUES (?,?,?,?,?,?)",
                (room, 0.0, 0.0, "Unpaid", json.dumps([]), datetime.now().isoformat())
            )
            self.conn.commit()
    ...
    def _reset_db_for_room(self, room):
        self.rooms_data[room] = {"items": [], "total_amount": 0.0, "total_paid": 0.0, "payment_status": "Unpaid"}
        self.cursor.execute(
            "UPDATE billing SET total_amount=?, total_paid=?, payment_status=?, items_json=?, last_updated=? WHERE room=?",
            (0.0, 0.0, "Unpaid", json.dumps([]), datetime.now().isoformat(), room)
        )
        if self.cursor.rowcount == 0:
            self.cursor.execute(
                "INSERT INTO billing (room, total_amount, total_paid, payment_status, items_json, last_updated) VALUES (?,?,?,?,?,?)",
    ...
    # (Hàm reset phía trên đã gọi _reset_all)
